...
var (
	...
	tokenizer  tokenizer.Tokenizer //TODO, WARNING: This may be not thread safe
...
func init() {
	...
	tokenizer = tokenizer.NewTreebankWordTokenizer()
}

func ModelAProcessor(input bwStruct.BWData) bwStruct.BWData {
	tokens := tokenizer.Tokenize(strings.ToLower(input.Input)) //WARNING this may be not thread safe
